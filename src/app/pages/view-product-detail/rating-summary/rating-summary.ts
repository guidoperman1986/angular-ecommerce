  ...
  template: `
    <div class="flex items-center gap-8 mb-6 p-4 bg-gray-50 rounded-lg">
      ...
      <div class="flex-1 mr-5">
        @for (breakdown of ratingBreakdown(); track $index) {
          <div class="flex items-center gap-2 mb-2">            
            <span class="text-sm w-4 flex items-center"> {{breakdown.stars}} <span class="ml-1 text-gray-400">★</span> </span>
...
export class RatingSummary {
  ...
  ratingBreakdown = computed(() => {
    ...
    return [5, 4, 3, 2, 1].map(stars => ({
      ...
      percentage: (reviews.filter(review => review.rating === stars).length / total) * 100
    }));
  });
}
